...
class main_UI(tk.Tk):  # 主視窗
    def __init__(self):
        ...
        menu.add_command(label='開啟音樂', command=self.openfile)
    ...
    def setup(self):  # 主視窗內容設定
        row1 = tk.Frame(self)
        row1.pack(fill="x")
        tk.Label(row1, text='處理進度：', font=20, width=10).pack(side=tk.LEFT)
        self.l3 = tk.Label(row1, textvariable=self.state, font=20, width=20)
        self.l3.pack(side=tk.LEFT)
        tk.Label(row1, text='音樂檔案：', font=15, width=10).pack(side=tk.LEFT)
        self.l0 = tk.Label(row1, textvariable=self.path, font=20, width=50)
        self.l0.pack(side=tk.LEFT)

        row2 = tk.Frame(self)
        row2.pack(fill="x")
        tk.Label(row2, text='節拍：', font=20, width=10).pack(side=tk.LEFT)
        self.l1 = tk.Label(row2, text=self.tempo, width=45)
        self.l1.pack(side=tk.LEFT)
        tk.Label(row2, text='每分鐘速度：', font=20, width=20).pack(side=tk.LEFT)
        self.l2 = tk.Label(row2, text=self.speed, width=40)
        self.l2.pack(side=tk.LEFT)

        row3 = tk.Frame(self)
        row3.pack(fill='x')
        tk.Button(row3, text='開啟音樂', width=13, command=self.openfile).pack(padx=6, side=tk.LEFT)
        tk.Button(row3, text='開始辨識', width=13,command=lambda:thread_it(self.identify)).pack(padx=15, pady=0, side=tk.LEFT)
        tk.Button(row3, text='顯示樂譜', width=13, command=self.display).pack(padx=6, side=tk.LEFT)
        tk.Button(row3, text='撥放音樂', width=13, command=play_music).pack(padx=6, side=tk.LEFT)
        tk.Button(row3, text='暫停音樂', width=13, command=pause_music).pack(padx=6, side=tk.LEFT)
        tk.Button(row3, text='繼續撥放', width=13, command=unpause_music).pack(padx=6, side=tk.LEFT)
        tk.Button(row3, text='設定', width=13, command=self.setup_config).pack(padx=6, side=tk.LEFT)
    ...
    def setup_config(self):  # 抓取子視窗資料
        ...
        fr.write(self.speed)
        fr.write(" ")
        fr.write(self.tempo[0])
        fr.write(" ")
        fr.write(self.tempo[2])
        fr.write('\n')
        fr.close()
        simple = self.tempo[0]
